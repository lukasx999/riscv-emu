...
void test_decoder_fence_type(std::string instruction, InstructionFence::Type type,
                               Register rd, Register rs1) {
    auto raw_inst = encode_instruction(std::move(instruction));
    auto inst = Decoder::decode(raw_inst->front());
    REQUIRE(std::holds_alternative<InstructionFence>(inst));
    auto i = std::get<InstructionFence>(inst);
    REQUIRE(i.type == type);
    REQUIRE(i.rd == rd);
    REQUIRE(i.rs1 == rs1);
}
...
TEST_CASE("decoder") {
    ...
    SECTION("rtype") {
        ...
        test_decoder_rtype("srlw t2, t1, t0", Srlw,  T2, T1, T0);
    ...
    SECTION("fence-type") {
        test_decoder_fence_type("fence", Fence, X0, X0);
    }
